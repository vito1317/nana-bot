    ...
    for idx, s_id in enumerate(servers):
        if server_id == s_id:
            ...
            if channel:
                ...
                conn_user_join = sqlite3.connect(f"./databases/analytics_server_{server_id}.db")
                ...
                responses = model.generate_content([
                    ...
                    f'新人審核格式包誇(```我叫:\n我從這裡來:\n我的困擾有:\n為什麼想加入這邊:\n我最近狀況如何：```)，example(僅為範例，請勿照抄):(你好！歡迎加入荒謬冒險團，很高興認識你！我叫奈奈，是你們的心理支持輔助機器人。如果你有任何情感困擾、生活問題，或是需要一點建議，都歡迎在審核後找我聊聊。我會盡力以溫暖、理解的方式傾聽，並給你專業的建議和支持。但在你跟我聊天以前，需要請你先到 <#{newcomer_review_channel_id}> 填寫以下資訊，方便我更好的為你服務！ ```我叫:\n我從這裡來:\n我的困擾有:\n為什麼想加入這邊:\n我最近狀況如何：```)請記住務必傳送>> ```我叫:\n我從這裡來:\n我的困擾有:\n為什麼想加入這邊:\n我最近狀況如何：```和<#{newcomer_review_channel_id}> <<'])

                if channel:
                    embed = discord.Embed(title="歡迎加入", description=responses[0].text)
    ...
    for idx, s_id in enumerate(servers):
        if server_id == s_id:
            ...
            if channel:
                ...
                conn_command = sqlite3.connect(f"./databases/analytics_server_{server_id}.db")
...
async def on_message(message):
    ...
    def init_db(db):
        ...
        c.execute(
            '''CREATE TABLE IF NOT EXISTS messages (user_id TEXT, user_name TEXT, channel_id TEXT, timestamp TEXT, content TEXT)''')  # Add messages table
        conn.commit()
        conn.close()
    ...
    def update_user_message_count(user_id, user_name, join_date):
        with sqlite3.connect("./databases/"+db_name) as conn:
            ...
            if result:
                ...
                c.execute('INSERT INTO users (user_id, user_name, join_date, message_count) VALUES (?, ?, ?, ?)',
    ...
    def store_message(user, content, timestamp):
        with sqlite3.connect("./databases/"+chat_db_name) as conn:
            c = conn.cursor()
            c.execute("INSERT INTO message (user, content, timestamp) VALUES (?, ?, ?)", (user, content, timestamp))
    ...
    def extract_command(text, command):
        match = re.search(fr'/{command}\s+(.+)', text)
    ...
    with sqlite3.connect("./databases/"+db_name) as conn:  # Use with statement for automatic closing
        c = conn.cursor()
        c.execute('''
    ...
    join_date = (message.author.joined_at + timedelta(hours=8)).replace(
        tzinfo=None).isoformat() if message.author.joined_at else None
    update_user_message_count(message.author.id, user_name, join_date)

    await bot.process_commands(message)

    if ('奈奈' in message.content) or (message.channel.id in TARGET_CHANNEL_ID) or (
            bot.user.mentioned_in(message) and not message.author.bot):
        try:
            if message.guild and message.guild.id not in WHITELISTED_SERVERS:
                await message.reply(f'Message from non-whitelisted server: {message.guild.id}')
                return
            ...
            async with message.channel.typing():
                ...
                if rows:
                    for row in rows:
                        if row[1]:
                            ...
                            messages = f"{row[2]} {row[0]}:{row[1]}" if row[0] != "奈奈" else row[1]
                            chat_history.insert(2, {"role": role, "parts": [{"text": messages}]})
                        else:
                            print(f'{row[1]} is null')
                else:
                    chat_history.insert(2, {"role": "user", "parts": [
                        {"text": "No previous conversation found."}]})  # Or some other default message

                print(chat_history)
                chat = model.start_chat(history=chat_history)
                response = chat.send_message(f"{get_current_time_utc8()} {user_name}:{message.content}")
                ...
                try:
                    response = chat.send_message(
                        get_current_time_utc8() + " " + user_name + ':' + message.content,
                        safety_settings={
            ...
            if search_query:
                ...
                reply = reply.replace(f"/search {search_query}", f"請稍後，正在搜尋{engine}...")
                await message.reply(reply)
            elif browse_url:
                reply = reply.replace(f"/browse {browse_url}", "請稍後，正在瀏覽網站...")
                await message.reply(reply)
            else:
                await message.reply(reply)
                ...
                if engine and query:
                    ...
                    async with message.channel.typing():
                        ...
                        with sqlite3.connect("./databases/"+f'messages_chat_{server_id}.db') as conn:
                            ...
                            conn.commit()
                        chat_history.append({"role": "user", "parts": [{"text": message.content}]})
                        chat = model.start_chat(history=chat_history)
                        print(chat_history)
                        try:
                        ...
                        time.sleep(1)
                        response = chat.send_message(
                            get_current_time_utc8() + "請總結 搜尋結果 " + query + ':' + content,
                            safety_settings={
                                HarmCategory.HARM_CATEGORY_HATE_SPEECH: HarmBlockThreshold.BLOCK_NONE,
                                HarmCategory.HARM_CATEGORY_HARASSMENT: HarmBlockThreshold.BLOCK_NONE,
                                HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT: HarmBlockThreshold.BLOCK_NONE,
                        ...
                        if match:
                            ...
                            print("Match not found.")
                        responses = response.text.replace(f"/search {engine} {query}", f"")
                        max_length = 2000
                        chunks = [content[i:i + max_length] for i in range(0, len(content), max_length)]

                        for chunk in chunks:
                            embed = discord.Embed(title="查詢結果: " + query, description=chunk,
...
bot.run('MTI3ODk2NTU2NDQwODMzMjM3MA.GMc8vf.bjCBtZaiAL0nP9OA1PNOgYc_IPcfwIMXC_X4S4')